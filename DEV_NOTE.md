...
- **两个路由**：`/api/og`（抓取→解析→图片元信息→校验，返回结构化报告）与 `/api/fetch`
  （通用代理，带 CORS 回传原始内容，供将来客户端工具跨域取数）。均 `GET`，**不写
  `export const runtime`**：OpenNext 把所有 server 面打进一个 worker 跑在 workerd，`runtime="edge"`
  是 Vercel 专用、在此 no-op。短缓存 `s-maxage=60`，配 UI「重新检测」(`fresh` 参数) 强制刷新。
