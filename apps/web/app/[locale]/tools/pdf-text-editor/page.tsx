...
export default async function PdfTextEditorPage({
  ...
  const structuredData = [
    ...
    },
    {
      ...
      name: content.pdfTextEditor.content.stepsTitle,
      step: tool.steps.map((step, index) => ({
        "@type": "HowToStep",
        name: `${content.pdfTextEditor.content.stepsTitle} ${index + 1}`,
        text: step,
        url: createLocalizedUrl(locale, tool.href),
      })),
      totalTime: "PT3M",
  ...
  return (
    <>
        ...
        <div className="relative mx-auto grid max-w-6xl gap-8 px-4 py-14 sm:px-6 lg:grid-cols-[minmax(0,1.2fr)_minmax(280px,0.8fr)] lg:py-18">
          <div className="space-y-5">
            <div className="flex flex-wrap gap-2">
              <Badge variant="yellow">
                {content.pdfTextEditor.hero.badges.category}
              </Badge>
              <Badge variant="info">
                {content.pdfTextEditor.hero.badges.localProcessing}
              </Badge>
              <Badge variant="outline">
                {content.pdfTextEditor.hero.badges.beta}
              </Badge>
            </div>
          ...
          <Card className="border-2 border-ink shadow-press-ink">
            <CardHeader className="border-b border-rule bg-paper-deep/50">
              <CardTitle>{content.pdfTextEditor.scenarios.title}</CardTitle>
              <CardDescription>
                {content.pdfTextEditor.scenarios.description}
              </CardDescription>
            </CardHeader>
            <CardPanel className="space-y-4 text-sm text-ink-soft leading-relaxed">
              <div className="flex gap-3">
                ...
                <p>{content.pdfTextEditor.scenarios.privacy}</p>
              </div>
              <div className="flex gap-3">
                <SparklesIcon className="mt-0.5 size-4.5 shrink-0 text-yellow-deep" />
                <p>{content.pdfTextEditor.scenarios.edit}</p>
              </div>
              <div className="flex gap-3">
                <LockIcon className="mt-0.5 size-4.5 shrink-0 text-yellow-deep" />
                <p>{content.pdfTextEditor.scenarios.fontReuse}</p>
              </div>
            </CardPanel>
          </Card>
        </div>
      ...
      <section className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:py-14">
        <PdfTextEditorClient content={content.pdfTextEditor} />
      </section>
      ...
      <section className="border-t border-rule bg-fluff/30">
        <div className="mx-auto grid max-w-6xl gap-6 px-4 py-16 sm:px-6 lg:grid-cols-[minmax(0,1fr)_minmax(0,0.9fr)]">
          <div className="space-y-6">
            <ContentCard
              description={content.pdfTextEditor.content.supportDescription}
              items={tool.features}
              title={content.pdfTextEditor.content.supportTitle}
            />
            <ContentCard
              description={content.pdfTextEditor.content.stepsDescription}
              items={tool.steps}
              ordered
              title={content.pdfTextEditor.content.stepsTitle}
            />
          </div>
          ...
          <div className="space-y-6">
            <Card>
              <CardHeader>
                <CardTitle>
                  {content.pdfTextEditor.content.limitsTitle}
                </CardTitle>
                <CardDescription>
                  {content.pdfTextEditor.content.limitsDescription}
                </CardDescription>
              </CardHeader>
              <CardPanel className="space-y-3 text-ink-soft text-sm leading-relaxed">
                {content.pdfTextEditor.content.limitsItems.map((item) => (
                  <p key={item}>{item}</p>
                ))}
              </CardPanel>
            </Card>
            ...
            <Card>
              <CardHeader>
                <CardTitle>{content.pdfTextEditor.content.faqTitle}</CardTitle>
                <CardDescription>
                  {content.pdfTextEditor.content.faqDescription}
                </CardDescription>
              </CardHeader>
