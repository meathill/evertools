/**
 * Print CSS for the "shadcn-typeset" style (the default). Two parts:
 * 1. A `:root` token preamble — the print popup never loads the app's
 *    globals.css, so these are copied verbatim from app/globals.css's
 *    `:root` block (2026-07) instead of relying on typeset's own
 *    color-mix() fallbacks, so printed output matches the live preview.
 * 2. The vendored app/typeset.css rules, flattened: the two
 *    `@layer components { }` wrappers are removed (safe here since this
 *    isolated document has no other Tailwind layers to lose priority
 *    to — app/typeset.css keeps the real `@layer` for the live app,
 *    where it matters for utility-class override order).
 *
 * To refresh part 2: fetch https://ui.shadcn.com/typeset.css, copy it
 * into app/typeset.css unmodified, then mirror the same content here
 * with only the two `@layer components {` / matching closing `}` lines
 * removed. Do not hand-edit anything else.
 */
export const SHADCN_TYPESET_PRINT_CSS = `
:root {
  --color-foreground: #3a2e23;
  --color-muted-foreground: #8a7660;
  --color-border: #e0d3b8;
  --color-muted: #ede2c5;
  --color-primary: #e6c34a;
  --color-ring: #e6c34a;
  --radius: 6px;
  --font-sans: "Nunito", -apple-system, BlinkMacSystemFont, "PingFang SC", "Hiragino Sans GB", "Microsoft YaHei", "Segoe UI", system-ui, sans-serif;
  --font-heading: "Fraunces", "Iowan Old Style", "Hoefler Text", "Source Serif Pro", Cambria, "Times New Roman", serif;
...
/* Print-specific fix: typeset's own "pre" rule uses overflow-x: auto (a
 * scroll container), which is correct on screen but clips long lines
 * when printed — print media can't scroll. Force wrapping for print
 * only, mirroring how the "classic" style already handles this. */
@media print {
  .typeset pre {
    overflow-x: visible;
...
export const SHADCN_TYPESET_BODY_CLASS_NAME = "typeset typeset-md-pdf";
